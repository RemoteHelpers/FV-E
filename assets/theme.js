...
theme.Header = (function() {
  ...
  function init() {
    ...
    cache.nav.addEventListener('click', function(e){
    	let liActive = target.querySelector(".site-nav--active");
		let aActive = liActive.querySelector(".site-nav__link--active");
		liActive.classList.remove('site-nav--active');
		aActive.classList.remove('site-nav__link--active');
    });
...
const contactFormSection = document.querySelector(".contactform-section.shopify-section_inner");
...
/*---------Animation Home Page Anna--------*/
const mainContentWrapper = document.querySelector('#MainContent');
if(mainContentWrapper){

  	const stepsWrapper = mainContentWrapper.querySelector('.shopify-section_inner.steps');
	const contactHelpWrapper = mainContentWrapper.querySelector('.index-section.contentHelp');

  if(stepsWrapper || contactHelpWrapper){   
  	document.addEventListener('scroll', function (e) {
  	const top  = window.pageYOffset + window.innerHeight;
    const isVisibleContactHelp = top > contactHelpWrapper.offsetTop;
  	const isVisibleStep = top > stepsWrapper.offsetTop + 400;
  	if (isVisibleStep) {
       const scrollSteps = stepsWrapper.querySelectorAll('.step_item');
       scrollAnimation(scrollSteps);
   	}
      
    if(isVisibleContactHelp){
  		const contactHelp = contactHelpWrapper.querySelector('#contact-help');	
      	contactHelp.classList.add('animate');
    }
...
function scrollAnimation(steps){
	let top1 = window.pageYOffset + window.innerHeight;
  	steps.forEach(step => {
      if(top1 > step.offsetParent.offsetTop + step.offsetTop+250){
      		step.classList.add('visible-color');
      }
    });
}

// Sergey: for sliders on product pages
$(document).ready(function(){
  $('.collection-slider').each(function(){
    var num_slides = 3;
    $(this).slick({slidesToShow:num_slides, dots: true, dotsClass: "my-dots",   infinite: true,
    slidesToShow: 3,
    slidesToScroll: 3,
    arrows: true,
    centerMode: true,
...
$(document).ready(function(){
  $('.slider-slick1').each(function(){
    $(this).slick({arrows: false, dots: true, dotsClass: "my-dots", centerMode: true, accessibility: false, centerPadding: '10px', slidesToShow: 3, infinite: true,
    responsive: [
      {
        breakpoint: 1000,
        settings: {
